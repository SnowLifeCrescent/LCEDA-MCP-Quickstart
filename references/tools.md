...
| `component_place` | 引导放置已确认的器件列表 | `components`（必填，uuid + libraryUuid）, `timeoutSeconds`（30-180，默认 60）, `interactive`（可选，默认 true，opencode CLI 请设为 false） |
...
- `component_select` 和 `component_place` 默认需侧边栏交互（`interactive: true`），在 opencode CLI 等无图形界面环境请传 `interactive: false`，跳过侧边栏由 AI 自行处理
